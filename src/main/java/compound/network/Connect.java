...
import compound.interfaces.Socket;
import processing.Talk;
...
public class Connect implements Socket {
    ...
    private Talk hello = null;
    private Request request = null;

    public Connect(SocketChannel client, Dns dns, Selector selector) throws IOException {
        this.dns = dns;
    ...
    private void clientRead(SelectionKey key) throws IOException {
        switch (state) {
            case HELLO:
                hello = Reader.readHelloMessage(this);
                if (hello == null) {
                    return;
                }
            ...
            case REQUEST:
                request = Reader.readRequestMessage(this);
                if (request == null) {
                    return;
                }
    ...
    private void clientWrite(SelectionKey key) throws IOException {
        switch (state) {
            case HELLO:
                if (writeBuff == null) {
                    writeBuff = ByteBuffer.wrap(Reader.getResponse());
                }
